...
allowed-tools: Bash(git *), Bash(gh *), Bash(az *), Bash(aws *), Bash(gcloud *), Bash(npm *), Bash(pytest *), Bash(python *), Read, Grep, Glob
...
Capture release scope, source branch/tag/commit, artifacts and versions, target environments, approvals required, and rollback strategy.
...
Use `references/templates.md` for `STAGE_DEPLOY_REPORT`, `PROD_PROMOTION_SIGNOFF`, `ROLLBACK_REPORT`, and `RELEASE_COMPLETION_CHECKLIST`.

## Output

When complete, report release plan, artifact metadata, stage deploy result, smoke-check evidence, production promotion approval/result if performed, and rollback status if invoked.
